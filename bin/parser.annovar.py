...
def cohort_update(n_variants, terms, samples, others, available):
    ...
    CohortModel.objects.filter(id=cohort_id).update(queries='[]', n_variants=n_variants)
...
def parse_row(cohort_id, infos, reader, samples):
    counter = 0
    ...
    for row in reader:
        ...
        for field in terms_available:
            if field in info_keys:
                if len(full_terms[field]) > BUFFER_SIZE:
                    ...
                    full_terms[field].add(row['INFO'][field])
        ...
        # ready to dump
        row['Others'] = json.dumps(row['Others'])
        del row['INFO']
        variants.append(VariantModel(**dict(row)))
        counter += 1
...
if '__main__' == __name__:
    ...
    terms, others, variants_length = parse_row(
        cohort_id, infos, reader, samples)
    print(
